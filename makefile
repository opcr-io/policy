...
# 	@${EXT_BIN_DIR}/golangci-lint run --config ${PWD}/.golangci.yaml
	@go work edit -json | jq -r '.Use[].DiskPath' | xargs -I{} bash -c 'cd {} && ${EXT_BIN_DIR}/golangci-lint run --config ${PWD}/.golangci.yaml'

.PHONY: test
test: 
	@echo -e "$(ATTN_COLOR)==> $@ $(NO_COLOR)"
	@go work edit -json | jq -r '.Use[].DiskPath' | xargs -I{} bash -c '${EXT_BIN_DIR}/gotestsum --format short-verbose -- -count=1 -parallel=1 -v -timeout=240s -coverprofile=cover.out -coverpkg={}/... {}/...'
...
go-mod-tidy:
	@echo -e "$(ATTN_COLOR)==> $@ $(NO_COLOR)"
	@go work edit -json | jq -r '.Use[].DiskPath' | xargs -I{} bash -c 'cd {} && echo {} && go mod tidy -v && cd -'
