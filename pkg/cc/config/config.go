...
func NewConfig(configPath Path, log *zerolog.Logger, overrides Overrider) (*Config, error) {
	...
	if err := func() error {
		if err := cfg.Logging.ParseLogLevel(zerolog.Disabled); err != nil {
	...
	if err := cfg.LoadDefaultDomain(); err != nil {
...
func (c *Config) SaveDefaultDomain() error {
	if _, err := os.Stat(c.FileStoreRoot); err != nil {
		if err := os.Mkdir(c.FileStoreRoot, x.FileMode0600); err != nil {
			return err
		}
	}

	return os.WriteFile(filepath.Join(c.FileStoreRoot, defaultDomain), []byte(c.DefaultDomain), x.FileMode0600)
}
...
func fileExists(path string) (bool, error) {
	...
	case os.IsNotExist(err):
		return false, nil
	case fi.IsDir():
		return false, errors.Errorf("%q: is a directory", path)
	default:
		return false, errors.Wrapf(err, "failed to stat file %q", path)
	}
}
